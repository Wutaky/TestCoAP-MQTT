...
class TempAndHumiResource(resource.CoAPResource):
    def __init__(self):
        resource.CoAPResource.__init__(self)
        self.visible = True
        self.observable = True
    ...
    def notify(self):
        log.msg('TempAndHumiResource: trying to send notifications')
        self.updatedState()
        reactor.callLater(60, self.notify)
    ...
    def render_GET(self, request):
        ...
        #     print "Error: " + str(e)
        # self.tempAndHumi = datetime.datetime.today().strftime("%Y-%m-%d %H:%M:%S") + "\ntemp = %.01fC\nhumidity = %.01f%%"%(temp, hum)
        response = coap.Message(code=coap.CONTENT, payload=self.tempAndHumi)
        return defer.succeed(response)

    def render_PUT(self, request):
        self.tempAndHumi = request.payload
        print 'PUT payload: ' + request.payload
...
root.putChild('temp-and-humi', TempAndHumiResource())
